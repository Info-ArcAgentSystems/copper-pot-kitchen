...
| Unit tests | **984 pass**, 2 skipped, 2 todo (`npm run test`) — 0.6s, no network |
...
| **`confidence` is a label, not a gate** | **decision needed** | Nothing in `src/engine` reads it — `grep -rn "confidence" src/engine/` returns nothing. A recipe saved as `confirm` feeds `requirementsForRange`, `productionBuckets` and `jobFoodCost` exactly like a `locked` one, so the badge on the Recipes list says "needs confirming" about a recipe already driving the shopping list. **Option B, deliberately NOT taken during the menu-search build:** make the engine exclude unconfirmed recipes and surface them as a gap. It is the right general fix, and it would retroactively pull Paul's existing SCANNED recipes out of shopping — silently, which is the bug class the whole August run was spent eliminating. So it needs to be his decision and its own change, with the migration of existing rows thought through. The menu search sidesteps it entirely by never writing a row |
...
| a schema that asked the model to calculate | The invoice tool described `lineTotal` as "IN CENTS — EUR 45.00 is 4500" inside a prompt whose headline is "you do not calculate". The model resolved the contradiction toward the louder instruction and reported the printed figure, which the validator then threw away — every number unreadable, every name fine. Two compounding defects: the contradiction, and narrowers demanding `typeof === 'number'` when models routinely quote numbers. **Worse than the visible failure: a plain `45.0` PASSED and was stored as 45 cents.** A hundredfold error that never showed as an error. Field renamed `lineTotalPrinted` so a stale deployed function fails VISIBLY rather than feeding cents into a euros parser during the deploy window. |
