...
function howMuch(data: SousData, asked: string, from: string, to: string): HowMuch {
  // The SHARED matcher (engine/nameMatch). The scanner resolves customers and
  // properties through the same function, so the two features cannot disagree
  // about whether a name is already in the owner's data.
  const matches = matchByName(data.ingredients, asked);
